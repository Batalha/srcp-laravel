...
String.prototype.toCamel = function () {
    retorne this.replace (/ (\ - [az]) / g, função ($ 1) {retorne $ 1.toUpperCase (). replace ('-', '');});
};
String.prototype.fromCamel = function () {
    return this.replace (/ ([AZ]) / g, função ($ 1) {return "-" + $ 1.toLowerCase ();});
};
...
função bufferToHex (arr) {
    var i;
    var len;
    var hex = '';
    var c;

    para (i = 0, len = arr.length; i <len; i + = 1) {
        c = arr [i] .toString (16);
        if (c.length <2) {
            c = '0' + c;
        }
...
função hexToBuffer (hex) {
    // TODO usa Uint8Array ou ArrayBuffer ou DataView
    var i;
    ...
    if (byteLen! == parseInt (byteLen, 10)) {
        throw new Error ("Comprimento hexadecimal inválido '" + hex.length + "'");
    }

    arr = novo Uint8Array (byteLen);

    para (i = 0; i <byteLen; i + = 1) {
...
// Funções de Conveniência Hex
window.Unibabel.hexToBuffer = hexToBuffer;
window.Unibabel.bufferToHex = bufferToHex;

function utf8ToBinaryString (str) {
    var escstr = encodeURIComponent (str);
    // substitui qualquer sequência de escape uri, como% 0A,
    // com escape binário, como 0x0A
    var binstr = escstr.replace (/% ([0-9A-F] {2}) / g, função (correspondência, p1) {
...
function binaryStringToUtf8 (binstr) {
    var escstr = binstr.replace (/(.)/g, função (m, p) {
        código var = p.charCodeAt (p) .toString (16) .toUpperCase ();
        if (code.length <2) {
            código = '0' + código;
        }
        retornar '%' + código;
...
função bufferToBinaryString (buf) {
    var binstr = Array.prototype.map.call (buf, função (ch) {
...
function binaryStringToBuffer (binstr) {
    ...
    if ('indefinido' === typeof Uint8Array) {
        buf = new Uint8Array (binstr.length);
    } outro {
        buf = [];
    }

    Array.prototype.forEach.call (binstr, função (ch, i) {
...
// Construção de blob para download
function base64toBlob (base64Data, contentType, sliceSize) {
    ...
    // Obter dados de blobs fatiados ou não
    blobData = sliceSize? getBlobDataSliced ​​(): getBlobDataAtOnce ();
    blob = new Blob (blobData, {tipo: contentType});
    return blob;
    / *
    * Obtenha dados blob em uma fatia.
    * => Rápido no IE no novo Blob (...)
    * /
    function getBlobDataAtOnce () {
        byteNumbers = new Array (byteCharacters.length);
        para (var i = 0; i <byteCharacters.length; i ++) {
    ...
    / *
    * Obtenha dados de blobs em várias fatias.
    * => Lento no IE no novo Blob (...)
    * /
    function getBlobDataSliced ​​() {
        var fatia,
            byteArrays = [];
        para (var offset = 0; deslocamento <byteCharacters.length; offset + = sliceSize) {
            slice = byteCharacters.slice (offset, offset + sliceSize);
            byteNumbers = new Array (slice.length);
            para (var i = 0; i <slice.length; i ++) {
...
função saver (url, winMode, fileName, defaultMime) {
    var anchor = document.createElement ("a");
    if ('download' na âncora) {// html5 A [download]
        anchor.href = url;
        anchor.setAttribute ("download", fileName);
        anchor.className = "download-js-link";
        anchor.innerHTML = "baixando ...";
        anchor.style.display = "nenhum";
        document.body.appendChild (âncora);
        setTimeout (function () {
            anchor.click ();
            document.body.removeChild (âncora);
            if (winMode === true) {setTimeout (function () {self.URL.revokeObjectURL (anchor.href);}, 250);}
        }, 66);
        retorno verdadeiro;
    }
    // lidar com safári não-a [download] da melhor maneira possível:
    if (/ (Version) \ / (\ d +) \. (\ d +) (?: \. (\ d +))?. * Safari \ //. teste (navigator.userAgent)) {
    if (/ ^ dados: /. teste (url)) url = "dados:" + url.replace (/ ^ dados: ([\ w \ / \ - \ +] +) /, defaultMime);
    if (! window.open (url)) {// popup bloqueado, oferecer download direto:
        if (confirm ("Exibindo o novo documento \ n \ nUsar Salvar como ... para fazer o download e, em seguida, clique em voltar para retornar a esta página.")) {location.href = url; }
    }
    retorno verdadeiro;
}
// faz download de dados do iframeURL (old ch + FF):
var f = document.createElement ("iframe");
document.body.appendChild (f);
if (! winMode && / ^ data: /. test (url)) {// forçar um mime a fazer o download:
...
xepOnline.IE = function () {
    ...
    if (msie> 0) {
        // IE 10 ou mais antigo => retornar o número da versão
        return parseInt (ua.substring (msie + 5, ua.indexOf ('.', msie)), 10);
    }

    if (tridente> 0) {
        // IE 11 => retornar o número da versão
        var rv = ua.indexOf ('rv:');
        return parseInt (ua.substring (rv + 3, ua.indexOf ('.', rv)), 10)> = 11;
    }

    if (aresta> 0) {
        // IE 12 => retornar o número da versão
        return parseInt (ua.substring (borda + 5, ua.indexOf ('.', borda)), 10);
    }
...
// TODO: melhor verificação de celular
xepOnline.detectmob1 = function () {if (navigator.userAgent.match (/ Android / i) || navigator.userAgent.match (/ webOS / i) || navigator.userAgent.match (/ iPhone / i) || navegador .userAgent.match (/ iPad / i) || navigator.userAgent.match (/ iPod / i) || navigator.userAgent.match (/ BlackBerry / i) || navigator.userAgent.match (/ Windows Phone / i) ) {return true;} else {return false;}}
xepOnline.mobilecheck = function () {
    var check = false;
    (função (a) {if (/ (android | bb \ d + | meego). + móvel | avantgo | bada \ / | amora | blazer | compal | elaine | fennec | hipopótamo | iemobile | ip (hone | od) | iris | kindle | lge | maemo | midp | mmp | mobile. + firefox | netfront | opera m (ob | in) i | palm (os)? | telefone | p (ixi | re) \ / | arrancador | bolso | psp | série (4 | 6) 0 | symbian | treo | up \. (navegador | link) | vodafone | wap | janelas (ce | telefone) | xda | xiino / i.test (a) || / 1207 | 6310 | 6590 | 3gso | 4thp | 50 [1-6] i | 770s | 802s | a wa | abac | ac (er | oo | s \ -) ​​| ai (ko | rn) | al (av | ca | co) | amoi | an (ex | ny | yw) | aptu | ar (ch | go) | como (te | us) | attw | au (di | \ -m | r | s) | avan | be (ck | ll | nq ) | bi (lb | rd) | bl (ac | az) | br (e | v) w | bumb | bw \ - (n | u) | c55 \ / | capi | ccwa | cdm \ - | célula | chtm | cldc | cmd \ - | co (mp | nd) | craw | da (it | ll | ng) | dbte | dc \ -s | devi | dica | dmob | do (c | p) o | ds (12 | \ -d) | el (49 | ai) | em (l2 | ul) | er (ic | k0) | esl8 | ez ([4-7] 0 | os | wa | ze) | fetc | fly (\ - | _) | g1 | g560 | gene | gf \ -5 | g \ -mo | go (\.w | od) | gr (anúncio | un) | haie | hc | hd | - | m | p | t | | hei \ - | hi (pt | ta) | hp (i | ip) | hs \ -c | ht (c (\ - | | _ | a | g | p | s | t) | tp) | hu (aw | tc) | i \ - (20 | ir | ma) | i230 | iac (| \ - | \ /) | ibro | idéia | ig01 | ikom | im1k | inno | ipaq | iris | ja (t | v) | jbro | jemu | jigs | kddi | keji | kgt (| \) | klon | kpt | kwc \ - | kyo (c | k) | le (no | xi) | lg (g | \ / (k | l | u) | 50 | 54 | \ - [aw]) | libw | lynx | m1 \ -w | m3ga | m50 \ | | ma | te | | | | | | 01 | 02 | bi | de | faça | t (\ - | | o | v) | zz) | mt (50 | p1 | v) | mwbp | mywa | n10 [0-2] | n20 [2-3] | n30 (0 | 2) | n50 (0 | 2 | 5) | n7 (0 (0 | 1) | 10) | ne ( (c | m) \ - | on | tf | wf | wg | wt) | nok (6 | i) | nzph | o2im | op (ti | wv) | oran | owg1 | p800 | pan (a | d | t ) | pdxg | pg (13 | \ - ([1-8] | c) | phil | pire | pl (ai | uc) | pn \ -2 | po (ck | rt | se) | prox | psio | pt \ -g | qa \ -a | qc (07 | 12 | 21 | 32 | 60 | \ - [2-7] | i \ -) ​​| qtek | r380 | r600 | raks | rim9 | ro (ve | zo ) | s55 \ / | sa (ge | ma | mm | ms | ny | va) | sc (01 | h \ - | oo | p \ -) ​​| sdk \ / | se (c (\ - | 0 | 1 ) | 47 | mc | nd | ri | | sgh \ - | shar | sie (\ - | m) | sk \ -0 | sl (45 | id) | sm (al | ar | b3 | it | t5) | então (ft | ny) | sp (01 | h \ - | v \ - | v) | sy (01 | mb) | t2 (18 | 50) | t6 (00 | 10 | 18) | ta (gt | lk ) | tcl \ - | tdg \ - | tel (i | m) | tim \ - | t \ -mo | para (pl | sh) | ts (70 | m \ - | m3 | m5) | tx \ -9 | up (\ .b | g1 | si) | ut | v400 | v750 | v | vk | (52 | 53 | 60 | 61 | 70 | 80 | 81 | 83 | 85 | 98) | w3c (\ - |) | webc | whit | wi (g | nc | nw) | wmlb | wuu | x700 | yas \ - | seu | zeto | zte \ - / i.teste (a) || / 1207 | 6310 | 6590 | 3gso | 4p | 50 [1-6] i | 770s | 802s | a wa | abac | ac (er | oo | s \ -) ​​| ai (ko | rn ) | al (av | ca | co) | amoi | an (ex | ny | yw) | aptu | ar (ch | go) | como (te | us) | attw | au (di | \ -m | r | s) | avan | ser (ck | ll | nq) | bi (lb | rd) | bl (ac | az) | br (e | v) w | bumb | bw \ - (n | u) | c55 \ / | capi | ccwa | cdm \ - | célula | chtm | cldc | cmd \ - | co (mp | nd) | craw | da (it | ll | ng) | dbte | dc \ -s | devi | dica | dmob | do (c | p) o | ds (12 | \ -d) | el (49 | ai) | em (l2 | ul) | er (ic | k0) | esl8 | ez ([4-7] 0 | os | wa | ze | | fetc | fly (\ - | _) | g1 | g560 | gene | gf \ -5 | g \ -mo | go (\.w | od) | gr (anúncio | un) | haie | hc | hd | - | m | p | t | | hei \ - | hi (pt | ta) | hp (i | ip) | hs \ -c | ht (c (\ - | | _ | a | g | p | s | t) | tp) | hu (aw | tc) | i \ - (20 | ir | ma) | i230 | iac (| \ - | \ /) | ibro | idéia | ig01 | ikom | im1k | inno | ipaq | iris | ja (t | v) | jbro | jemu | jigs | kddi | keji | kgt (| \) | klon | kpt | kwc \ - | kyo (c | k) | le (no | xi) | lg (g | \ / (k | l | u) | 50 | 54 | \ - [aw]) | libw | lynx | m1 \ -w | m3ga | m50 \ | | ma | te | | | | | | 01 | 02 | bi | de | faça | t (\ - | | o | v) | zz) | mt (50 | p1 | v) | mwbp | mywa | n10 [0-2] | n20 [2-3] | n30 (0 | 2) | n50 (0 | 2 | 5) | n7 (0 (0 | 1) | 10) | ne ( (c | m) \ - | on | tf | wf | wg | wt) | nok (6 | i) | nzph | o2im | op (ti | wv) | oran | owg1 | p800 | pan (a | d | t ) | pdxg | pg (13 | \ - ([1-8] | c) | phil | pire | pl (ai | uc) | pn \ -2 | po (ck | rt | se) | prox | psio | pt \ -g | qa \ -a | qc (07 | 12 | 21 | 32 | 60 | \ - [2-7] | i \ -) ​​| qtek | r380 | r600 | raks | rim9 | ro (ve | zo ) | s55 \ / | sa (ge | ma | mm | ms | ny | va) | sc (01 | h \ - | oo | p \ -) ​​| sdk \ / | se (c (\ - | 0 | 1 ) | 47 | mc | nd | ri | | sgh \ - | shar | sie (\ - | m) | sk \ -0 | sl (45 | id) | sm (al | ar | b3 | it | t5) | então (ft | ny) | sp (01 | h \ - | v \ - | v) | sy (01 | mb) | t2 (18 | 50) | t6 (00 | 10 | 18) | ta (gt | lk ) | tcl \ - | tdg \ - | tel (i | m) | tim \ - | t \ -mo | para (pl | sh) | ts (70 | m \ - | m3 | m5) | tx \ -9 | up (\ .b | g1 | si) | ut | v400 | v750 | v | vk | (52 | 53 | 60 | 61 | 70 | 80 | 81 | 83 | 85 | 98) | w3c (\ - |) | webc | whit | wi (g | nc | nw) | wmlb | wuu | x700 | yas \ - | seu | zeto | zte \ - / i.test (a.substr (0,4))) verificar = verdadeiro}) (navigator.userAgent || navigator.vendor || window.opera);
    return xepOnline.detectmob1 () || Verifica;
}
...
// TODO: melhor método de ignorar mídia, talvez picar a folha de estilo css para verificar se esta é a mídia de "bootstrap.css" a ser ignorada
...
xepOnline.Formatter = {
    clean_tags: ['img', 'hr', 'br', 'entrada', 'col', 'embed', 'param', 'source', 'link'],
    fo_attributes_root: [
        'cor',
        'altura',
    ...
    getRealStyle: function (elm, atributos, pseudo) {
        var returnObj = {};
    ...
    handlePseudoElem: function(dest) {
        jQuery(dest).find('*').each(function(index) {
            ...
            if(before.getPropertyValue('content').length > 0 && before.getPropertyValue('content') != "none"){
                ...
                before_text.forEach(function (part){
                    ...
                    if (part.indexOf ("ttp: //")> = 0 && in_image == true) {
                        var img = jQuery ('<img src = "url (& quot;' + parte + '& quot;)">');
            ...
            if(after.getPropertyValue('content').length > 0 && after.getPropertyValue('content') != "none"){
                ...
                after_text.forEach (função (parte) {
                    if (part == "")
                        processado = true;
                    if (part.indexOf ("url (")> = 0) {
                        in_image = true;
